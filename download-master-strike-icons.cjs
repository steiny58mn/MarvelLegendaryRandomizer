...
async function downloadAll() {
  const res = await fetch('https://master-strike.com/js/app.11c3eb2e.js');
  const text = await res.text();
  const matches = [...text.matchAll(/\"(img\/([^\.\"]+)\.([a-f0-9]+)\.svg)\"/g)];
  
  console.log(`Found ${matches.length} SVGs to download from master-strike.com`);

  for (const m of matches) {
    const fullUrl = `https://master-strike.com/${m[1]}`;
