export const GAME_KEYWORDS = [
  ...
  { name: "Command", rule: "A Villain “Commands” their group if it is the leftmost Villain of that group in the city. (Even if it is the only card of that group.)", matchPattern: '\\bCommand(s|ing)?\\b(?!\\s+Strike)' },
  ...
  { name: "Support", rule: "Support Heroes are secondary Heroes that provide additional effects.", matchPattern: '\\bSupport\\s+Hero(es)?\\b' },
  ...
  { name: "Mission", rule: "Missions act similarly to Villains but represent objectives. If a Mission escapes the city, it increases the Danger Level by 1.", matchPattern: '\\bMissions?\\b' },
