    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-2 sm:gap-3 border-b border-slate-800 pb-2 mb-2 overflow-x-auto">
        <button 
          ...
          className={`min-h-[42px] px-3.5 text-xs sm:text-sm font-bold uppercase tracking-wide border-b-2 transition-colors flex items-center gap-2 whitespace-nowrap ${activeTab === 'rules' ? 'border-purple-500 text-purple-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
        >
        ...
        <button 
          ...
          className={`min-h-[42px] px-3.5 text-xs sm:text-sm font-bold uppercase tracking-wide border-b-2 transition-colors flex items-center gap-2 whitespace-nowrap ${activeTab === 'keywords' ? 'border-purple-500 text-purple-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[60vh] overflow-y-auto pr-2">
            {GAME_KEYWORDS.sort((a, b) => a.name.localeCompare(b.name)).map((kw, i) => (
              <div key={i} className="p-4 bg-slate-950/60 rounded-xl border border-slate-800 hover:border-slate-700 transition-colors">
                <h3 className="text-base font-extrabold text-purple-400 mb-1">{kw.name}</h3>
