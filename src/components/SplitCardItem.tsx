      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 border-b border-slate-750/60 pb-2">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {half.heroClass && half.heroClass.split(',').map((cls: string, cIdx: number) => (
          ...
          <div className="flex items-center gap-1.5 text-xs font-bold shrink-0 self-start sm:self-auto">
            ...
            {half.piercing !== undefined && half.piercing !== null && half.piercing !== '' && (
              <span className="px-2 py-0.5 rounded bg-fuchsia-950/70 text-fuchsia-300 border border-fuchsia-800/40 inline-flex items-center gap-1 font-bold">
                <span>⚡ {half.piercing}</span>
              </span>
            )}
          </div>
        </div>
        ...
        <div className="pt-0.5 text-xs sm:text-sm">
          {half.rulesText ? (
            ...
            <div className="text-slate-500 text-xs italic">No rules text (base stats only)</div>
          )}
        </div>
      </div>
    </div>
...
export const SplitCardItem: React.FC<SplitCardItemProps> = ({ card, idx, splitData }) => {
  ...
  return (
    <div
      key={idx}
      className="bg-slate-800/60 border-2 border-indigo-500/40 hover:border-indigo-400/60 rounded-xl p-3.5 sm:p-4 flex flex-col gap-3 select-text shadow-lg transition-all"
    >
      {/* Top Banner / Collapsible Bar for the Split Card Container */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 border-b border-indigo-500/30 pb-2.5">
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          <span className="inline-flex items-center gap-1 text-[11px] uppercase font-black tracking-wider px-2 py-0.5 rounded-md bg-indigo-950 text-indigo-300 border border-indigo-700/60 shadow-sm">
            <Split className="w-3.5 h-3.5 text-indigo-400" />
            Divided / Split Card
          </span>
          <h4 className="font-extrabold text-slate-100 text-base sm:text-lg tracking-wide truncate">
            {card.name}
          </h4>
        </div>

        {/* Header Controls: Collapsible window toggle & artwork indicator */}
        <div className="flex items-center gap-2 text-xs font-semibold self-end sm:self-auto">
          <button
            type="button"
            onClick={() => setIsExpanded(!isExpanded)}
            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-700/60 hover:bg-slate-700 text-slate-200 hover:text-white border border-slate-600/60 transition-colors cursor-pointer"
            aria-label={isExpanded ? "Collapse split card window" : "Expand split card window"}
          >
            <span className="text-xs font-medium">{isExpanded ? "Collapse View" : "Expand Halves"}</span>
            {isExpanded ? <ChevronUp className="w-3.5 h-3.5 text-indigo-300" /> : <ChevronDown className="w-3.5 h-3.5 text-indigo-300" />}
          </button>
        </div>
      </div>

      {/* Collapsible content: Side-by-side or stacked distinct card halves */}
      {isExpanded && (
        <div className="flex flex-col gap-3 animate-fade-in">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <HalfCardView half={left} badgeColor="cyan" />
            <HalfCardView half={right} badgeColor="purple" />
          </div>

          {/* Full Split Card Artwork Toggle */}
          {card.imageUrl && (
            <div className="mt-1 pt-2 border-t border-slate-700/40 flex flex-col gap-2 select-none">
              <button
                ...
                className="inline-flex items-center gap-1.5 text-xs text-amber-400 hover:text-amber-300 font-semibold self-start py-1 px-2.5 rounded-lg hover:bg-slate-700/50 transition-colors cursor-pointer select-none"
              >
                <ImageIcon className="w-3.5 h-3.5" />
                <span>{showImage ? 'Hide Divided Card Scan' : 'View Full Divided Card Scan'}</span>
                ...
                >
                  {!isLoaded && !imgError && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-slate-950/95 z-10 select-none p-4 text-center">
                      ...
                      <span className="text-xs text-slate-300 font-medium tracking-wide">Loading divided card artwork...</span>
                    </div>
                  )}
                  {imgError ? (
                    ...
                    <img
                      ...
                      className={`w-full h-full object-contain rounded-xl transition-opacity duration-150 select-none pointer-events-none ${
