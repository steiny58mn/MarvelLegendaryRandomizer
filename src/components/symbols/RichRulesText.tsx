...
const KEYWORD_PATTERN =
  '\\b(?:Master Strike|Command Strike|Scheme Twist|Plot Twist|When Recruited|Ambush Schemes|Ambush|Fight|Escape|Rescue|Strike|Bystander Rescue|Wound a Villain|Wounded Fury|Wound|Bribe|Bribery|Berserk(?:\\s+\\d+)?|Wall[ -]?Crawl|Teleport|Versatile(?:\\s+\\d+)?|Focus(?:\\s+\\d+)?|Phasing|Microscopic\\s+Size[ -]?Changing|Size[ -]?Changing|Smash(?:\\s+\\d+)?|Elusive(?:\\s+\\d+)?|Excessive\\s+(?:Violence|Kindness)|X[ -]?Treme\\s+Attack|X-Gene|(?:Double\\s+)?Dark\\s+Memories|Hyperspeed|Thrown\\s+Artifacts?|Ritual\\s+Artifacts?|Triggered\\s+Artifacts?|Artifacts?|Undercover|Transforms?|Transformed|Transforming|Transforming\\s+(?:Heroes|Masterminds|Schemes)|Haunt|Heist|Patrol|Explore|Digest|Indigestion|Demolish|Dodge|(?:Double|Triple|Quadruple\\s+)?Empowered(?:\\s+by)?|Soaring\\s+Flight|Soulbind|Spectrum|(?:Double|Triple\\s+)?Striker|Sunlight|Moonlight|Switcheroo(?:\\s+\\d+)?|Symbiote\\s+Bonds|Tactical\\s+Formation|Piercing\\s+Energy|Worthy|Cosmic\\s+Threat|Coordinate|Clone|(?:[A-Za-z-]+[ -])?Conqueror(?:\\s+\\d+)?|Cross[ -]?Dimensional\\s+[A-Za-z0-9\\\'\\s-]+Rampage|Cross[ -]?Dimensional\\s+Rampage|Cyber-Mod|Danger\\s+Sense|Demonic\\s+Bargain|Dominate|Double-Cross|Endgame|Fated\\s+Future|Fateful\\s+Resurrection|Feast|Fortify|Hidden\\s+Witnesses?|Human\\s+Shields?|Hunt\\s+for\\s+Victims|HYDRA\\s+Level|Investigate|(?:Double\\s+)?Last\\s+Stand|Liberate(?:\\s+\\d+|\\s+X)?|Lightshow|(?:Man|Woman)\\s+Out\\s+of\\s+Time|(?:Mass\\s+)?Momentum(?:\\s+\\d+)?|Outwit|Prey|Revenge|Rise\\s+of\\s+the\\s+Living\\s+Dead|Sacrifice|Savior|(?:Cosmic\\s+)?Shards?|Shatter|S\\.H\\.I\\.E\\.L\\.D\\.\\s+Clearance|S\\.H\\.I\\.E\\.L\\.D\\.\\s+Level|Throne[\\\'\u2019]s\\s+Favor|Uru[ -]?Enchanted\\s+Weapons?|Villainous\\s+Weapons?|Waking\\s+Nightmare|(?:Doubled\\s+)?Weapon\\s+X\\s+Sequence|What\\s+If(?:\\.\\.\\.\\?)?|\\d+(?:st|nd|rd|th)?\\s+Circle\\s+of\\s+(?:Kung-Fu|Quack-Fu)|Circle\\s+of\\s+(?:Kung-Fu|Quack-Fu)|Abomination|Antics|Astral\\s+Plane|Blood\\s+Frenzy|Burrow|Celestial\\s+Boon|Charge|Cheering\\s+Crowds|Chivalrous\\s+Duel|Contest\\s+of\\s+Champions|Unveiled|Veiled|Adapting(?:\\s+Masterminds?)?|Acid\\s+Blood|Facehugger|Chestburster|Bullet\\s+Time|Gadget|Valyrian\\s+Steel|Wildfire|Trophy|Black\\s+Oil|Hellmouth|Dust|Job|Scan|Hope|Fear|Support\\s+Hero(?:es)?|Danger\\s+Level|Missions?|Ascend|Divided(?:\\s+Cards)?|Special\\s+S\\.H\\.I\\.E\\.L\\.D\\.\\s+Officers?|Sidekicks?|Locations?|Traps?|Command(s|ing)?(?!\\s+Strike))\\b';
