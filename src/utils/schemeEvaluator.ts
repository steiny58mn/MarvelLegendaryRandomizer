import { SchemeCard } from '../types';

// Community consensus curated ratings for notable Legendary schemes
const COMMUNITY_TIERS: Record<string, 'Easy' | 'Moderate' | 'Hard' | 'Extreme'> = {
...
/**
 * Evaluates the accurate difficulty rating for a Scheme card using community tiers
 * and algorithmic rule text analysis.
 */
export function evaluateSchemeDifficulty(scheme: Partial<SchemeCard> | any): 'Easy' | 'Moderate' | 'Hard' | 'Extreme' {
  if (!scheme) return 'Moderate';

  const normId = normalizeKey((scheme.id || '').replace(/^scheme-/, ''));
  const normName = normalizeKey(scheme.name || '');

  // Check direct community matches
  ...
  // Algorithmic evaluation based on card rules
  const allText = [
    scheme.name || '',
    scheme.setupRule || '',
    scheme.twistEffect || '',
  ...
  if (/evil wins:\s*when\s*([2-4])\s*(villains?|masterminds?|twists?|bystanders?|heroes)/i.test(allText)) {
    ...
    score -= 10; // Standard villain deck timer
  }

  // 3. Brutal game disruption
